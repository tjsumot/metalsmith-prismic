        ...
        function processSingleFragment(fragment, ctx, queryMetadata) {
          ...
          if (fragment instanceof Prismic.Fragments.DocumentLink && fragment.document.data) {
            ...
            fragmentObject.children = _.map(fragment.value, function(sliceFragment) {
              var processFragment = function (frag) {
                const subFragmentObject = processSingleFragment(frag, ctx, queryMetadata);
              ...
              var nonRepeat = _.mapObject(sliceFragment.nonRepeat, function (s) {
                return processFragment(s);
              });

              var repeat = _.map(sliceFragment.repeat, function (s) {
                return _.map(s, function (s) {
                  return _.mapObject(s.fragments, processFragment);
                })
              });
              ...
              return {
                repeat: repeat,
                nonRepeat: nonRepeat,
                sliceType: sliceFragment.sliceType,
                sliceLabel: sliceFragment.label
              };
            });
          }

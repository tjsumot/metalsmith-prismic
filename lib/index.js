    ...
    return function(files, metalsmith, done){
        ...
        function processSingleFragment(fragment, ctx, queryMetadata) {
          ...
          if (fragment instanceof Prismic.Fragments.DocumentLink && fragment.document.data) {
            ...
            // TODO
            fragmentObject.children = _.flatten(_.map(fragment.value, function(sliceFragment) {
              const fragments = [];
              if (sliceFragment.value || sliceFragment.nonRepeat && sliceFragment.nonRepeat.value) {
                var val = sliceFragment || sliceFragment.nonRepeat.value;
                ...
                fragments.push(subFragmentObject)
              }

              if (sliceFragment.repeat && sliceFragment.repeat.value) {
                fragments.push.apply(fragments, _.map(sliceFragment.repeat.value, function(subFrag) {
                  var subFragmentObject = processSingleFragment(subFrag, ctx, queryMetadata);
                  subFragmentObject.sliceType = sliceFragment.sliceType;
                  subFragmentObject.sliceLabel = sliceFragment.label;
